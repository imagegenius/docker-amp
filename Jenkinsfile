pipeline {
  ...
  environment {
    ...
    REPO = 'docker-amp'
    CONTAINER_NAME = 'amp'
    DOCKERHUB_IMAGE = 'hydaz/amp'
    DEV_DOCKERHUB_IMAGE = 'dev/amp'
    PR_DOCKERHUB_IMAGE = 'pipepr/amp'
    DIST_IMAGE = 'alpine'
    MULTIARCH = 'false'
    CI = 'false'
    CI_WEB = 'false'
  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sL https://cubecoders.com/AMPVersions.json | jq -r '.AMPCore' ''',
            returnStdout: true).trim()
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/hydazz/docker-amp/packages\" \
          --label \"org.opencontainers.image.source=https://github.com/hydazz/docker-amp\" \
          ...
          --label \"org.opencontainers.image.description=[AMP (Application Management Panel)](https://cubecoders.com/AMP) - a simple to use and easy to install control panel and management system for hosting game servers.  \" \
